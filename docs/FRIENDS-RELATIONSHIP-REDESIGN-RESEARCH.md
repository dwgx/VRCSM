...
Last updated: 2026-06-23 (research baseline). **Status 2026-08-21:** ego-graph seeds self from `world_visits` (**shipped v0.16.3**). Mutuals API is fetchOne in the inspector; **no persist tables, no startup crawl**. Successor pack: `docs/SESSION-HANDOFF-2026-08-21.md`. Local VRCX clone `D:\Reference\VRCX` is `5ea37a2` — ideas only, no paste.

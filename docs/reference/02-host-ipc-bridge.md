...
`MainWindow` 持 `unique_ptr<WebViewHost>`；`WebViewHost` 构造时创建 `unique_ptr<IpcBridge>`；`IpcBridge` 持 `WebViewHost&` 反向引用。IPC 处理函数实现分散在 `bridges/*.cpp`（21 个文件，含 MusicBridge/LyricsBridge），`RegisterHandlers()`（`IpcBridge.cpp:607`）只做"方法名 → lambda"接线。
...
- `src/host/bridges/*.cpp`（21 个）、`bridges/BridgeCommon.h`

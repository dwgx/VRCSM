...
版本一致性：`VERSION`、`web/package.json`、`vcpkg.json`、README badge/artifact 名必须一起 bump（`scripts/verify-release-metadata.ps1` 会检查）。当前 `0.16.1`。改 `VERSION` 后必须 `cmake --preset x64-release` reconfigure，否则 ninja 不重建 `app.rc`。

...
Result<nlohmann::json> Database::FavoriteItems(const std::string& list_name)
{
    ...
    {
        nlohmann::json row = nlohmann::json::object();
        row["type"] = ColumnTextOrNull(rawStmt, 0);
        row["target_id"] = ColumnTextOrNull(rawStmt, 1);
        row["list_name"] = ColumnTextOrNull(rawStmt, 2);
        row["display_name"] = ColumnTextOrNull(rawStmt, 3);
...
Result<int> Database::ImportFavoriteList(const nlohmann::json& payload)
{
    ...
    int schemaVersion = 0;
    if (!JsonObjectInt(payload, "schema_version", schemaVersion) || schemaVersion != 1)
    {
        return MakeError("db_invalid_argument", "schema_version must be 1");
    }

...
- **GUI↔API contract audit + remediation (2026-07-09).** A full read-only audit of the IPC boundary (185 handlers × 128 call sites) drove a batch of correctness/robustness/security fixes, each locked by a test: the lyrics proxy's SSRF rail now refuses redirect + DNS-rebind bypasses (and rejects CRLF in the referer); `settings.readAll`/`config.read` and friends surface real errors instead of resolving an `{error}` blob (fixed a Registry-tab white-screen); transient VRChat API errors (429/5xx/network) no longer flap the app into a logged-out state and wipe caches; whole-number OSC float params keep their `,f` type across the wire (VRChat was dropping them); `junction.repair` now refuses to run while VRChat is open; the updater's MSI download moved off the 60s timeout tier and requires https; `images.cache`/`thumbnails.fetch` batches are chunked so items past the host cap aren't dropped; co-presence analytics and the activity heatmap now parse VRChat's dotted timestamp format (they were empty/mis-grouped for all real data); ~19 handlers now return stable machine-readable error codes instead of a generic `handler_error`, rowid parameters no longer truncate at 2³¹, and no-op writes on a missing id report `not_found` instead of false success; and plugin callers no longer receive the full self-account document from `auth.user` or a zero-permission filesystem-layout probe. Also hardened the self-update apply step: the installer now runs via a detached bootstrap that waits for the app to exit before replacing it, verified on a real 0.14.6→0.14.7 hot-update.
...
This release is the current checkpoint before pausing active development. It wraps up the recent UI, cache, updater, migration, and OSC work into a single release so the shipped build matches the current code instead of leaving a long-running unreleased branch.

- **Migration, updater, and database repair work shipped together.** The `migrate.execute` IPC path no longer double-dispatches and leave the UI stuck in `running`, `world_visits` now dedupes old rows before the unique index is created, updater IPC failures are real IPC errors again, and installer application is constrained to validated files under the VRCSM updates directory with matching size/hash metadata.
- **OSC Studio moved from raw sender toward a usable tool surface.** The page now has concrete starter templates, live preview, component insertion, a visual composer, clearer manual-send/auto-send controls, explicit card state, and better card mutation handling so auto-send no longer races newly created or edited cards.
- **Chatbox send behavior is now aligned with VRChat's actual contract.** `/chatbox/input` sends arguments in the correct order (`message`, `sendImmediately`, `playNotificationSound`), auto-send renders at actual send time, and the sender serializes against Chatbox pacing so the UI no longer claims to be sending while VRChat quietly drops or ignores messages.
- **OSC errors are now diagnosable instead of opaque.** Invalid IPv4 input, socket creation failures, and `sendto()` failures come back as structured IPC errors, so the frontend can show a real reason instead of a bare `ERROR`.
- **Hardware telemetry coverage improved without inventing values.** GPU source/adapter details, SMBIOS motherboard/RAM identity fallback, AIDA64 shared-memory sensors, and ACPI thermal-zone fallback all ship in this release, but missing live sensors are still treated as missing rather than fabricated.
- **Friend, world, and social surfaces were tightened for real usage.** Friends removed the misleading avatar/model filter, social analytics now excludes the signed-in user from encounter rankings, and heavy world rows/thumbs are mounted or prefetched more lazily so large histories do not immediately flood the UI.
- **Asset metadata is now reusable across pages.** The new `asset_cache` table and shared frontend cache path let world/avatar/user names and images be reused across maps, world history, popup badges, and inspector surfaces instead of every page rediscovering the same metadata independently.
- **Release-facing cleanup was folded in.** The about-dialog acknowledgement card was removed, packaging no longer depends on a single global WiX installation path, real release asset filenames are respected by the updater, and the host/frontend artifacts were rebuilt and repackaged from the same state.

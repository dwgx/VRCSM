...
- **Hardware identity fallback.** `hw.telemetry` now reads raw SMBIOS (`GetSystemFirmwareTable('RSMB')`) as a fallback for motherboard and RAM module identity when CIM/WMI is slow, empty, or broken. Live temperature/fan/power values still require real sensor sources such as NVML, LibreHardwareMonitor/OpenHardwareMonitor WMI, AIDA64 shared memory, or future vendor SDKs; VRCSM does not fabricate missing sensor values.
...
- **Unified asset metadata cache.** A new `asset_cache` SQLite table and `assets.resolve` / `assets.prefetch` / `assets.invalidate` IPC path store reusable world, avatar, and user metadata with source/confidence tracking. World, avatar, and user popup badges now render cached names and images first, and existing VRChat API detail/friends calls backfill the cache instead of forcing each page to rediscover the same metadata.
- **Worlds page lazy image reuse.** Worlds now warms visible rows through the unified asset cache and queues the lookahead rows as low-priority asset prefetches, so local log/world history names and fetched thumbnails can be reused by maps, world history, social analysis, and inspector surfaces.
- **OSC Studio editor cleanup.** The template composer has a one-click editor clear button, and rendered Chatbox templates now drop pipe-separated segments that contain unavailable `--` values. Missing CPU temperature, fan, board, or sensor values no longer get sent as broken text when other parts of the template are valid.
- **OSC auto-send timing fix.** Chatbox auto-send now renders the template at the actual send moment after hardware refresh and rate-limit waits, and the scheduler runs as a single cancellable timeout loop instead of overlapping async `setInterval` jobs. Stopping auto-send also cancels in-flight rate-limit waits before they can send a stale message. Rendered templates now remove missing-value placeholders inside a segment, drop empty CPU/fan-only segments, and treat a lone category label like `Thermal` as no valid message instead of sending broken `--` text.
- **OSC auto-send and sensor visibility.** Every OSC Studio card now has its own visible Auto/Stop control, and the selected-template panel shows the active card, sent/skipped counters, next-send countdown, last rendered message, and current error. `hw.telemetry` now also reads AIDA64's public `AIDA64_SensorValues` shared-memory feed, classifies temperature/fan/power/load/voltage/clock rows, and shows the first live sensor readings in the hardware panel. Thermal defaults and old saved `Fan {gpu.fanPct}` templates migrate to `{fan.0}` so RPM-style fan sensors can render instead of `Fan --`.
- **Windows ACPI thermal-zone fallback.** `hw.telemetry` now queries `ROOT\WMI:MSAcpi_ThermalZoneTemperature` and converts its tenths-Kelvin values to Celsius, exposing them as `acpi_thermal_zone` readings. These are platform thermal-zone sensors, not guaranteed CPU core/package sensors, but they improve out-of-box temperature coverage on machines without NVML/AIDA64/LHM/OHM.
- **OSC Studio send-flow cleanup.** Template preset buttons now apply directly to the selected editor card instead of only appending new cards below. Default auto-send intervals were lowered to `1s`. Manual Chatbox sends now use a local `5 messages / 5 seconds` burst guard with explicit feedback, while auto-send no longer forces the old local 2-second wait path. `Send selected` and `Auto send` also now error clearly when no card is selected.

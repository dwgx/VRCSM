...
Last updated: 2026-08-22 (v0.16.5 release cut)
...
1. `docs/SESSION-HANDOFF-2026-08-22.md` — **present-tense acceptance pack**, subagent prompts, ranked backlog. Git wins if this file disagrees. `docs/SESSION-HANDOFF-2026-08-21.md` is the 0.16.4 wrap (historical).
...
5. `CHANGELOG.md` — release history and user-visible behavior. `[0.16.5]` is the current cut.
6. Only then inspect code. Then `git status --short --branch`.

## Current Continuity Snapshot

- **2026-08-22 v0.16.5 RELEASE.** Quality leftovers after 0.16.4: FTS-gate, I18N-grey, julian World History/UnifiedFeed, Invite Assist / Event Watch fire-time, Log `Debug` prefix, logout UI-thread cookies, LyricsProxy CGNAT/ULA. Encrypted UnityFS 3D still empty. No GPL/decrypt paste. LICENSE VSAL.
- **Verified 2026-08-22 (v0.16.5 cut):** `corepack pnpm@11.8.0 --dir web build` clean; x64-release ctest **255 passed / 0 failed** (5 live DISABLED + skips). FileVersion **0.16.5.0**. Artifacts: MSI `VRCSM_v0.16.5_x64_Installer.msi` (9,351,168 bytes, SHA256 `6174a30a8541a80b6b8cb6ae017ca03d63d878cafd0f3f93ef3e025f8463b6c6`), ZIP `VRCSM_v0.16.5_x64.zip` (21,850,459 bytes, SHA256 `f02ab9681762c2223eeae82c87fce92ff6db16678966ed5937d2fb3649943e0c`). `SHA256:` line is required in GitHub release notes.
- Do **not** commit `CLAUDE.md`, `AGENTS.md`, `.agent/`, or the scratch txt. Product source at HEAD is **committed** (`881368d`). This wrap may leave **tracked handoff docs** dirty until a `docs: record v0.16.5 release hashes` commit.
- **P18 / P16 / P4 / P12** in 0.16.0. **FL1 / VL1 / P15 / MU1 fetchOne / P2 / P5 / SR1 fold+table / UX1 / P17 `/migrate`** in 0.16.3. **GH-CI / CI1 / SR1-pop / MU1-db / crawl / P17 mocks / grey master ON / config.write rethrow** in **0.16.4**. **I18N-grey / FTS-gate / history julian / fire-time / Debug logs / logout COM** in **0.16.5**. Next named: **P6 / P7 / P14 / P25 / P1** or Owner-named. Do not recut 0.16.5. Confirm WIN-CI on `881368d`.
...
- Historical note: product HEAD is **0.16.5** (`881368d`). Still do NOT commit `2026-07-04-111708-...txt`. Control-plane `CLAUDE.md` / `AGENTS.md` / `.agent/` stay uncommitted.
- Release mechanics reminder: bump all of `VERSION`, `web/package.json`, README badge+artifact names together; **reconfigure the CMake preset after editing `VERSION`** (it's read at configure time — a plain rebuild says "ninja: no work to do"); the updater's fail-closed hash gate needs the `SHA256:` line in the GitHub release notes; **stop VRCSM before reinstalling** or the running WebView2 locks `web/` and the MSI silently keeps the stale bundle (a real trap hit this session). After `pnpm build`, copy `web/dist` into host `web/` if ninja reports no work.
- **Release-cut test baseline:** 0.16.5 is x64-release ctest **255/255** (5 live DISABLED + skips) + FileVersion **0.16.5.0**. 0.16.4 was 248/248 + Playwright 62/62. Full web vitest still needs `--no-file-parallelism`. Pre-existing compile noise: PluginBridge u8path C4996, CommonTests getenv C4996, sscanf C4996 OtpMailParser/ImapClient.

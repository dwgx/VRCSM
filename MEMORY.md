...
- **2026-08-20 v0.16.0 RELEASE.** Companion-tools wave (last-instance, OSC nits, visit-keyed screenshots, activity ledger, avatar presets, entity search, app-data backup, Slot mail, Playspace offset, SAPI TTS, IMAP OTP DPAPI default OFF, Invite Assist, Event Watch, Hot Worlds, `{hr.bpm}`, `cache_expiry_delay`, visit-time normalize, update-dialog phase fix). Grey helpers default OFF. LICENSE still VSAL. New UI i18n is `defaultValue` plus four sidebar keys. Do not commit the scratch txt at repo root.
- **Verified 2026-08-20 (release cut):** see the session that tagged `v0.16.0`. Pre-cut: `tsc -b` clean; `vite build` clean; jsdom pages-smoke 39 + interaction-smoke 37 = **76/76**; Playwright UI smoke **61/61**; x64-debug ctest **225/225** (6 live probes disabled/skipped). Full vitest parallel runner still flakes; use `--no-file-parallelism`. Version files: `VERSION` / `web/package.json` / `vcpkg.json` / README badge+artifacts at **0.16.0**. CMake must be **reconfigured** after the VERSION bump.
- Current branch: `main` after merging `feat/companion-waves`. Last previously released tag was **v0.15.2**. Historical v0.15.1 notes follow.

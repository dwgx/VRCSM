...
- **2026-08-20 companion-waves (unreleased on `feat/companion-waves`).** Last released tag is still **v0.15.2** (`ece11f3` / `ad132c5` on `main`). This branch adds last-instance, OSC nits, visit-keyed screenshots, activity ledger, avatar presets, entity search, app-data backup, Slot mail, Playspace offset, SAPI TTS, IMAP OTP (DPAPI, default OFF), Invite Assist, Event Watch, Hot Worlds, `{hr.bpm}`, `cache_expiry_delay`, and visit-time normalize. Grey helpers default OFF. LICENSE still VSAL. i18n for new UI is `defaultValue` plus four sidebar keys. Do not commit the scratch txt at repo root.
- **Verified 2026-08-20 (this branch):** `tsc -b` clean; `vite build` clean; jsdom pages-smoke 39 + interaction-smoke 37 = **76/76**; Playwright UI smoke **61/61** (34 click-through + 27 nav-visual); x64-debug `VRCSM_Tests` links; ctest debug **225/225** (6 live probes disabled/skipped). Full vitest parallel runner still flakes; use `--no-file-parallelism`. MSI not cut. VERSION remains `0.15.2` until a release bump.
- Current branch: `main`, **in sync with `origin/main`** (0 ahead / 0 behind) as of the v0.15.1 release. HEAD `88ef241` (release: sync vcpkg.json + complete 0.15.1 changelog). **Superseded for current git:** see the 2026-08-20 bullet. Historical v0.15.1 notes follow.

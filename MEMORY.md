...
- **2026-08-20 v0.16.0 RELEASE.** Companion-tools wave (last-instance, OSC nits, visit-keyed screenshots, activity ledger, avatar presets, entity search, app-data backup, Slot mail, Playspace offset, SAPI TTS, IMAP OTP DPAPI default OFF, Invite Assist, Event Watch, Hot Worlds, `{hr.bpm}`, `cache_expiry_delay`, visit-time normalize, update-dialog phase fix). Grey helpers default OFF. LICENSE still VSAL. **Follow-up on main:** companion keys filled in all 7 locales; coverage test is now 7-way + placeholder parity. Do not commit the scratch txt at repo root.
- **Verified 2026-08-20 (v0.16.0 cut):** `tsc -b` + `vite build` clean; jsdom smoke **76/76**; Playwright UI smoke **61/61**; x64-release `VRCSM.exe` FileVersion **0.16.0.0**; ctest release **225/225** (6 live probes disabled/skipped). Full vitest parallel runner still flakes; use `--no-file-parallelism`. Version files in sync at **0.16.0**. Artifacts: MSI `VRCSM_v0.16.0_x64_Installer.msi` (9,158,656 bytes, SHA256 `254745ce7f0a0709995bb00455c43139bfb6d3b976fa128b89d3b628b0f0f3bd`), ZIP `VRCSM_v0.16.0_x64.zip` (21,599,324 bytes, SHA256 `2522a11f1452944aba25062342d6e5230230ccfe4412f9e41c14d5e2549a05e7`). `SHA256:` line is required in GitHub release notes (updater fail-closed).

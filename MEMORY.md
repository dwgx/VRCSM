...
- Current branch: `main`, **7 commits ahead of `origin/main`, 0 behind** — HEAD `821f3b7`, NOT yet pushed. (The old "60 ahead / 12 behind Dependabot" is DONE — Dependabot was merged into the v0.15.0 release; those 7 unpushed are the post-0.15.0 patch commits below.)
- **v0.15.0 was released to GitHub** (tag `v0.15.0` + MSI/ZIP, 2026-07-09). **v0.15.1 is LOCAL-only** (`VERSION`/`web/package.json` = `0.15.1`), NOT pushed/released — it bundles: the i18n language-persistence fix, the QQ Music lyrics source, OSC progress/marquee sliders, the factory-reset thumbnail fix, and the `{music.lyrics}` send fix. A local 0.15.1 MSI is installed on this machine.
...
1. **Online lyric-source expansion for obscure/uploaded tracks.** LRCLIB/NetEase/QQ-search miss indie/user-uploaded songs (e.g. "恶口 1&2 / undaloop" — QQ finds it by TITLE-only but not title+artist). Two research workflows landed: (a) `lyric-source-expansion` (Kugou/Kuwo/Migu public APIs + a QQ title-only fallback rule) and (b) staleness audit. **Kugou has the widest obscure-track coverage** — integrate it as a 4th source (KRC is encrypted/zipped → needs a decode step, flagged higher-effort) plus the cheap QQ title-only fallback first. Read the workflow synthesis outputs under the session temp dir / re-run the saved scripts.

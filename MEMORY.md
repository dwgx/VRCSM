...
Last updated: 2026-08-21 (v0.16.4 release cut)
...
- **2026-08-21 v0.16.4 RELEASE.** GH-CI YAML, CacheIndex Bundles list, FTS populate, mutual persist + user-triggered crawl, plugin IPC mocks, grey master default ON (assist/watch/IMAP still confirm), config.write rethrows errors. Encrypted UnityFS 3D still empty. No GPL/decrypt paste. LICENSE VSAL.
- **Verified 2026-08-21 (v0.16.4 cut):** `tsc -b` + `pnpm build` clean; x64-debug and x64-release ctest **248 passed / 0 failed** (1 skipped, 5 live DISABLED); Playwright `pnpm test:ui-smoke` **62/62**. FileVersion **0.16.4.0**. Artifacts: MSI `VRCSM_v0.16.4_x64_Installer.msi` (9,342,976 bytes, SHA256 `018cd4e4a1ba4973670fcbb95f8d960133f647bad5293bbb3a72c10c75697363`), ZIP `VRCSM_v0.16.4_x64.zip` (21,838,741 bytes, SHA256 `2138528e0f38cc7a6cb021642f0d38469cb2a9477c0d04c8944f1442fc2c6736`). `SHA256:` line is required in GitHub release notes.
- Do **not** commit `CLAUDE.md`, `AGENTS.md`, `.agent/`, or the scratch txt. Product source at HEAD is committed; this wrap may leave **tracked handoff docs** dirty until Owner names a docs commit.
- **P18 / P16 / P4 / P12 shipped in 0.16.0.** **FL1 / VL1 / P15 / MU1 fetchOne / P2 / P5 / SR1 fold+FTS table / UX1 / P17 `/migrate` shipped in 0.16.3.** **GH-CI / VER1 / CI1 / SR1-pop / MU1-db / P17-B11 landed uncommitted** after that cut (still 0.16.3). Next: named commit or 0.16.4. GitHub Actions green after the YAML change is **unknown** until push.
...
- Historical note (v0.15.1 era): the tree was clean except the scratch txt. **As of the 0.16.3 wrap the product tree is committed** at `33c8e85`. Still do NOT commit `2026-07-04-111708-...txt`. Control-plane `CLAUDE.md` / `AGENTS.md` / `.agent/` stay uncommitted.
...
- **Release-cut test baseline:** 0.16.3 is ctest **239/239** (debug+release) + Playwright **62/62**. Historical: 0.16.0 was ctest **225/225** + Playwright **61/61**; 0.16.2 cut was ctest **229/229** + jsdom **76/76**. Full web vitest still needs `--no-file-parallelism`. GitHub Actions on `33c8e85` is **red** (Pages green; Ubuntu Vite onnxruntime postinstall timeout; Ubuntu Vitest `/tools/osc` 5s; Windows Release Node 20 + pnpm 11 `node:sqlite`). Release compile still emits PluginBridge.cpp:172 u8path C4996, CommonTests.cpp getenv C4996, plus sscanf C4996 in OtpMailParser/ImapClient and a wchar→char C4244 in LyricsProxy (pre-existing, not a ship blocker).
